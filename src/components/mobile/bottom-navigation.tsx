...
export function BottomNavigation({ activeTab, onTabChange, className }: BottomNavigationProps) {
  ...
  return (
    <div className={cn(
      ...
      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-blue-600 to-transparent transition-all duration-300"
        style={{ 
          opacity: activeTab ? 1 : 0,
          transform: `translateX(${(navItems.findIndex(item => item.id === activeTab) / (navItems.length - 1)) * 100}%)`
        }}
      />
    </div>
  )
